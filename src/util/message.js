...
export async function buildMessage(authPriv, dhPrivateKey, msgText, from, dhPublicKey) {
	const t = Date.now()
	const iv = await getIV(toHex(from)+t)
	const ivBytes = new Uint8Array(iv)
	const derivedKey = await deriveDHSecret(dhPublicKey, dhPrivateKey)
	const encrypted = await encrypt(iv, derivedKey, new TextEncoder().encode(msgText))
