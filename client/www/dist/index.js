...
var TodoList = class extends s4 {
  ...
  render() {
    ...
    return p`
      <h1>Todo List</h1>
      <ul>
        ${items.map((item) => p`<li class=${item.completed ? "completed" : ""} @click=${() => this.toggleCompleted(item)}>${item.text}</li>`)}
      </ul>
...
__publicField(TodoList, "properties", {
  listItems: {},
  hideCompleted: {}
});
__publicField(TodoList, "styles", r`
    li {
      font-size: 1.2rem;
    }
    .completed {
